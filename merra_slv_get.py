...
#retry behavior in case run into download problems
MAX_RETRIES = 3
RETRY_DELAY_SEC = 10
TIMEOUT_SEC = 120
...
def extract_urls(txt_path: str) -> list[str]:
    """Extract all HTTPS URLs from the text file I downloaded after selecting wanted 
    variables, time period, spatial, etc."""
...
def preprocess_local_file(fp: Path) -> xr.Dataset:
    """
    Open and downloaded local NetCDF file and keep only the wanted SLV vars.
    """
